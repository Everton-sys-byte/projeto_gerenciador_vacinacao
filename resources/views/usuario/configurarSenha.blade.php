...
@section('content')
    <div class="information container-fluid px-5">
        <x-titles.default-title title="Configurar Senha" />
        <x-tags.tag-bar class="mt-3"/>
        @include('template.defaultSessionAlert.alert')
        <x-form.default-form formAction="{{@route('user.update.password')}}" formMethod="POST" class="mx-auto" style="width: 500px">
            @method('PUT')
            ...
            <x-form.form-group label_name="Repetir nova senha" inputType="password" inputName="repetir_novaSenha" :inputValue="@old('repetir_novaSenha')"/>
